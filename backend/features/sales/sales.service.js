...
exports.addSaleTransaction = async (date, customerId, uniqueItems, ownerId, recordedBy) => {
    ...
    try {
        ...
        for (const p of itemsJson) {
            ...
            const stockQuery = queries.getRemainingStock(p.product_version_id, ownerId);
...
exports.deleteSaleItemTransaction = async (itemId, ownerId, userId, userRole) => {
    ...
    try {
        ...
        // 5. Restore stock
        const restoreQuery = queries.restoreItemStock(item.quantity, item.product_version_id, ownerId);
