...
function DashboardInner() {
    const { stock, sales, attendance, dashboardStats, isLoading, resetData, exportReport, showToast, fetchUsers } = useStore();
    ...
    }, [perm.isAdmin, fetchUsers]);

    const { lowStock, monthlySalesInfo, topSellerName, customerProfitArr, totalShakeProfit, shakeProfitArr, totalSalesProfit, totalVpSold, totalStockValue, totalStockVp } = useMemo(() => {
        ...
        if (dashboardStats) {
            return {
                ...
                totalStockVp: dashboardStats.totals?.totalStockVp || 0,
            };
        }
        ...
        let stockVpTotal = 0;
        safeStock.forEach(s => {
            stockValTotal += ((s?.sp || 0) * (s?.qty || 0));
            stockVpTotal += ((s?.vp || 0) * (s?.qty || 0));
        });
        ...
        return {
            ...
            totalStockValue: stockValTotal,
            totalStockVp: stockVpTotal,
        };
    ...
    return (
        <div>
            ...
            <div className="card-grid" style={{ padding: 0, marginBottom: 30 }}>
                ...
                <AdminMetricCard icon={Zap} title="Total Stock V.P" value={Number(totalStockVp || 0).toFixed(1)} color="#10b981" subtitle="Volume points" />
            ...
            {perm.isAdmin && (
                <div className="card" style={{ marginBottom: 30 }}>
                    <h3 style={{ marginBottom: 20 }}>Staff Session Status</h3>
                    <div className="table-container" style={{ margin: 0 }}>
                        <table>
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                {useStore.getState().users.length > 0 ? (
                                    useStore.getState().users.map(u => (
                                        <tr key={u.id}>
                                            <td><strong>{u.username}</strong></td>
                                            <td><span style={{ fontSize: 12, textTransform: 'capitalize', color: 'var(--text-light)' }}>{u.role}</span></td>
                                            <td className="text-right">
                                                {u.session_token ? (
                                                    <span style={{ color: '#16a34a', fontWeight: '600', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 5 }}>
                                                        <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#16a34a', display: 'inline-block' }}></span> Active Now
                                                    </span>
                                                ) : (
                                    ...
                                    <tr><td colSpan="3" style={{ textAlign: 'center', color: 'var(--text-light)', padding: 20 }}>No staff loaded.</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
