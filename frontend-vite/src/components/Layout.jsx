        ...
        <div className="app-container">
            ...
            <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
                ...
                <nav style={{ flex: 1, padding: '20px 0' }}>
                    {user?.role === 'master' ? (
                        <NavLink to="/master" className={({isActive}) => `nav-link ${isActive ? 'active':''}`} onClick={()=>setSidebarOpen(false)}><LayoutDashboard size={20}/> Master Panel</NavLink>
                    ) : (
                        <>
                            ...
                            <NavLink to="/settings" className={({isActive}) => `nav-link ${isActive ? 'active':''}`} onClick={()=>setSidebarOpen(false)}><Database size={20}/> Settings</NavLink>
                        </>
                    )}
                </nav>
                <div className="nav-link" style={{ color: 'var(--alert-color)', marginTop: 'auto', marginBottom: 20, cursor: 'pointer' }} onClick={handleLogout}>
